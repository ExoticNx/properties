...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(60000)
		MySQL.Async.fetchAll('Select * FROM tj_businesses', {}, function(result)
			MySQL.Async.fetchAll('Select * FROM tj_research', {}, function(result2)
				MySQL.Async.fetchAll('Select * FROM tj_business_storage', {}, function(result3)
					local found = false

					for i = 1, #result, 1 do
						for j = 1, #result3, 1 do
							if result[i].property == result3[j].property then
								if result[i].currentProduction == result3[j].name then
									if result3[j].amount > 0 then
										found = true
										print(found)
										break
									end
					...
					for i = 1, #result, 1 do
						if result[i].researchTimeLeft ~= nil then
							if result[i].researchTimeLeft > 0 then
								MySQL.Sync.execute("UPDATE tj_businesses SET researchTimeLeft=@researchTimeLeft WHERE researchTimeLeft > 0", {['@researchTimeLeft'] = result[i].researchTimeLeft - 1})
								break
							elseif result[i].researchTimeLeft == 0 then
								MySQL.Async.execute("INSERT INTO tj_completed_research (name, researchName) VALUES (@name,@researchName)", {['@name'] = result[i].name, ['@researchName'] = result[i].currentResearch})
								MySQL.Sync.execute("UPDATE tj_businesses SET currentResearch=@currentResearch WHERE researchTimeLeft = 0", {['@currentResearch'] = nil})
								MySQL.Sync.execute("UPDATE tj_businesses SET researchTimeLeft=@researchTimeLeft WHERE researchTimeLeft = 0", {['@researchTimeLeft'] = nil})
								break
							end
						end
					end
					...
					for i = 1, #result, 1 do
						if result[i].productionTimeLeft ~= nil then
							if result[i].productionTimeLeft > 0 then
								MySQL.Sync.execute("UPDATE tj_businesses SET productionTimeLeft=@productionTimeLeft WHERE productionTimeLeft > 0", {['@productionTimeLeft'] = result[i].productionTimeLeft - 1})
								break
							elseif result[i].productionTimeLeft == 0 then
								if found == true then
									for j = 1, #result3, 1 do
										if result[i].property == result3[j].property then
											if result[i].currentProduction == result3[j].name then
												if result3[j].amount > 0 then
													MySQL.Sync.execute("UPDATE tj_business_storage SET amount=@amount WHERE property = @property AND name = @name", {['@property'] = result3[j].property, ['@amount'] = result3[j].amount + result[i].currentProductionAmount, ['@name'] = result3[j].name})
													MySQL.Sync.execute("UPDATE tj_businesses SET currentProduction=@currentProduction WHERE productionTimeLeft = 0", {['@currentProduction'] = nil})
													MySQL.Sync.execute("UPDATE tj_businesses SET productionTimeLeft=@productionTimeLeft WHERE productionTimeLeft = 0", {['@productionTimeLeft'] = nil})
													MySQL.Sync.execute("UPDATE tj_businesses SET currentProductionAmount=@currentProductionAmount WHERE currentProductionAmount > 0", {['@currentProductionAmount'] = 0})
													break
												end
											end
										end
								...
								elseif found == false then
									for j = 1, #result2, 1 do 
										if result[i].currentProduction == result2[j].name then
											MySQL.Async.execute("INSERT INTO tj_business_storage (type, name, spawn, property, amount) VALUES (@type,@name,@spawn,@property,@amount)", {['@type'] = result[i].type, ['@name'] = result[i].currentProduction, ['@spawn'] = result2[j].spawn, ['@property'] = result[i].property, ['@amount'] = result[i].currentProductionAmount})
											MySQL.Sync.execute("UPDATE tj_businesses SET currentProduction=@currentProduction WHERE productionTimeLeft = 0", {['@currentProduction'] = nil})
											MySQL.Sync.execute("UPDATE tj_businesses SET productionTimeLeft=@productionTimeLeft WHERE productionTimeLeft = 0", {['@productionTimeLeft'] = nil})
											MySQL.Sync.execute("UPDATE tj_businesses SET currentProductionAmount=@currentProductionAmount WHERE currentProductionAmount > 0", {['@currentProductionAmount'] = 0})
											break
